...
def shop():
    for item in items:
        ...
        if "armor" in item:
             print(f"{item['armor']*100}% к уменьшению входяшего урона")
        if "hp" in item:
            print(f"{item['hp']} к здоровье")
        if "attack" in item:
            print(f"{item['attack']} +урон к атаке")
    print(f"Баланс: {player["money"]}")
    choice = int(input("Выберети предмет(1,2,3...)"))
    if choice >= len(items):
        choice = len(items)-1
    if choice < 1:
        choice = 1
...
def inventory():
    ...
    if item["mode"]== "unequipment":
        ...
        print(f"Вы ипользовали {item['name']}")
